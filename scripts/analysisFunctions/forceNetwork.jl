...
function forceNetwork(dataDirectory, centralCell, show)
    ...
    @unpack A,Aᵀ,B,Bᵀ,B̄,C,R,F,edgeTangents,edgeLengths,edgeMidpoints,cellPositions,ϵ,cellAreas,boundaryVertices,edgeLengths,externalF = matricesDict["matrices"]
    ...
    if length(cellVerticesDict[centralCell]) != length(neighbouringCells)
        @error "Central cell is on the system periphery. Select an internal cell."
        # # Boundary cell
        # pressureOnCell = sum(externalF[cellVerticesDict[centralCell]])
        # pressureAngle = atan(pressureOnCell...)
        # # Find angles of vertices around cell
        # vertexAngles = zeros(size(cellVerticesDict[centralCell]))
        # for (k,v) in enumerate(cellVerticesDict[centralCell])
        #     vertexAngles[k] = atan((R[v].-cellPositions[centralCell])...)
        # end
        # vertexAngles .+= 2π-pressureAngle
        # vertexAngles .= vertexAngles.%2π
        # # Sort vertices around cell by polar angle
        # cellVerticesDict[centralCell] .= cellVerticesDict[centralCell][sortperm(vertexAngles)]
        # neighbourAngles = zeros(length(neighbouringCells))
        # for (i,c) in enumerate(neighbouringCells)
        #     neighbourAngles[i] = atan((cellPositions[c].-cellPositions[centralCell])...)
        # end
        # neighbourAngles .+= (2π-pressureAngle)
        # neighbourAngles = neighbourAngles.%(2π)
        # neighbouringCells .= neighbouringCells[sortperm(neighbourAngles)]
        # for (i,v) in enumerate(cellVerticesDict[centralCell][1:end-1])
        ...
        save("/Users/christopher/Dropbox (The University of Manchester)/Chris-Oliver Shared/VertexModelFigures/$(splitdir(dataDirectory)[end])/pdf/cell$(centralCell)ForceNetwork.pdf",fig)
        save("$dataDirectory/svg/cell$(centralCell)ForceNetwork.svg",fig)
        save("/Users/christopher/Dropbox (The University of Manchester)/Chris-Oliver Shared/VertexModelFigures/$(splitdir(dataDirectory)[end])/svg/cell$(centralCell)ForceNetwork.svg",fig)
        save("$dataDirectory/png/cell$(centralCell)ForceNetwork.png",fig)
        save("/Users/christopher/Dropbox (The University of Manchester)/Chris-Oliver Shared/VertexModelFigures/$(splitdir(dataDirectory)[end])/png/cell$(centralCell)ForceNetwork.png",fig)
    end
