...
function model!(du, u, p, t)
    ...
    for k = 1:nVerts
        ...
        du[k] = 2 .*(sum(@view F[k, :]) .+ externalF[k]) 
    ...
    #@show test
    du[nVerts+1]=SVector(-sum((cellAreas.*(cellAreas.*u[nVerts+1][1].-1.0).+cellPerimeters.*(0.5.*ΓA .+ Γa.*(2.0.*cellAreas.+u[nVerts+1][1].*cellPerimeters.-a₀)))), 0.0)
    #@show du[nVerts+1]
    return du
end
