    ...
    while integrator.t<params.tMax
        ...
        if integrator.t%params.outputInterval<integrator.dt
            ...
            if frameDataToggle==1
                ...
                R = @view integrator.u[:]
                jldsave(datadir("sims",subFolder,folderName,"frames","data$(@sprintf("%03d", integrator.t*outputTotal÷params.tMax)).jld2");matrices,params,R)            
            ...
            frameImageToggle==1 ? save(datadir("sims",subFolder,folderName,"frames","frame$(@sprintf("%03d", integrator.t*outputTotal÷params.tMax)).png"),fig) : nothing
        end
